...
class SamplingTree_pywapper:
    def __init__(self, capacity):
        ...
        # get_sammple時にindicsを外に出さないための変数
        # get_sample毎にindicsをここに記録して，次回のupdate時に使用する
    ...
    def get_sample(self, sample_size: int) -> NDArray[Any, Float]:
        '''
        ...
        self._indics4update = indics

        return np.array(samples)
    
    def update(self, new_value: NDArray[Any, Float]) -> None:
        '''
        葉の値を更新する．
        updateを1回呼び出すためには，前回のupdate以降にget_sampleを呼び出している必要がある
        '''
        if self._indics4update == None:
            raise RuntimeError("you must call get_sample() before calling update()")
        
        self._tree_cpp.update(new_value.tolist(), self._indics4update)
        self._indics4update = None
